...
const TWITCH_OAUTH_URL="https://api.twitch.tv/kraken/oauth2/authorize";
...
export default class Twitch {
   ...
   static getInstance() {
    ...
    async getUserAccessToken() {
        ...
        //console.log(result);
        let resultat = await this.tokenValid(result.params.code);
        console.log(resultat);
        if(resultat) {
            this.access_token = result.params.code;
            this.getInfo();
        }
    }
    ...
    async tokenValid(token) {
      try {
        ...
        const response = await fetch(`${TWITCH_BASE_URL}`, {
          method: 'GET',
          headers: {
